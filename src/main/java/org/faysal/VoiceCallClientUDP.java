...
public class VoiceCallClientUDP {
    private static final String SERVER_ADDRESS = "10.15.18.130";
...
//            DatagramPacket packet = new DatagramPacket(buffer, buffer.length);
//            socket.receive(packet);
//            System.out.println("Received packet from server with length " + packet.getLength());
//            speakers.write(packet.getData(), 0, packet.getLength());
//        }
//    } catch (Exception e) {
//        e.printStackTrace();
    ...
    private void receiveAudio() {
        ...
        try {
            while (true) {
                DatagramPacket packet = new DatagramPacket(buffer, buffer.length);
                socket.receive(packet);
                System.out.println("Received packet from " + packet.getAddress() + ":" + packet.getPort() + " with length " + packet.getLength());
                speakers.write(packet.getData(), 0, packet.getLength());
            }
        } catch (Exception e) {
            e.printStackTrace();
